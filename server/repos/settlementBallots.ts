...
/*
 * The ONE reading of a ballot's outcome. A carried row a steward stopped can
 * still say `passed` on disk with `vetoed_at` or `landing_status = 'vetoed'`
 * beside it, and every reader must call that failed. Reading `status` alone
 * here would have shown a vetoed moon as carried: no warning on the card, and
 * the job waiting forever for a landing that will never come.
 */
import { outcomeStatusOf } from "../lib/ballots";
...
export async function settlementAsks(pool: Pool, cycleIds: readonly string[]): Promise<SettlementAsk[]> {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT subject_ref, status, landing_status, vetoed_at FROM ballots WHERE subject_type = ? AND subject_ref IN (${cycleIds.map(() => "?").join(",")})`,
    [CYCLE_SETTLEMENT, ...cycleIds],
  );
  return rows.map((r) => {
    const status = String(outcomeStatusOf(r));
    return {
...
export async function latestSettlementRefusals(
  ...
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT id, subject_ref, status, landing_status, closed_at, vetoed_at FROM ballots WHERE subject_type = ? AND subject_ref IN (${cycleIds.map(() => "?").join(",")}) ORDER BY created_at DESC, id DESC`,
    [CYCLE_SETTLEMENT, ...cycleIds],
  );
