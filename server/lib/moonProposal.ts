  ...
  return {
    ...
    execute: async (b: BallotRow) => {
      ...
      if (!result.ok) {
        ...
        /*
         * THROW, NEVER RETURN, WHEN NOTHING WAS SETTLED.
         *
         * `applyDue` marks a landing applied whenever a closer's `execute`
         * returns, and takes its failure path (the row goes back to pending and
         * the next tick tries again) only when it throws. Returning here, as this
         * first did, recorded a settlement the village carried as landed while
         * no moon closed and nobody was paid, and nothing would ever retry it.
         * server/lib/landingRefusal.ts states the same contract for the other
         * closers. The admins are told first, so a retrying failure is visible.
         */
        throw new Error(`The settlement the village passed could not land: ${result.error}`);
      }
      /*
