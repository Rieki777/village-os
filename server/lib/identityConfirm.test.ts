...
describe("the gate both destructive routes call", () => {
  ...
  it("answers a legacy unsalted hash by naming the reset, and never reaches the password check", async () => {
    ...
    // at all, while a session minted before that change still reaches here.
    //
    // The gate below is built with a verifyPassword that ALWAYS SAYS YES. If
    // the legacy branch were missing, this case would pass the gate and the
    // count would be 1, so the assertion is about the order and not only the
    // sentence.
    let asked = 0;
    const legacy = "0123456789abcdef".repeat(4);
    const gate = makeIdentityGate({
      authSecret: SECRET,
      verifyPassword: async () => { asked += 1; return true; },
      googleAvailable: () => true,
      members: { update: async () => null },
    });
    const row: any = { id: "user-g", passwordHash: legacy, tokenVersion: 0, prefs: {} };
    const req: any = { body: { password: "anything" }, headers: {} };
    const res: any = { clearCookie: () => {} };

    const out = await gate(req, res, row, "delete-account");

    expect(out).toEqual({ ok: false, body: { error: PASSWORD_NEEDS_RESET } });
    expect(asked, "the legacy branch answers before any password check").toBe(0);
    expect(PASSWORD_NEEDS_RESET).not.toBe(PASSWORD_REFUSAL["delete-account"]);
  });
