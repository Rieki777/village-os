 ...
 * `org_role_assignments` the way the settlement does, and that migration 0186
...
describe.skipIf(!configured)("the needs store", () => {
  ...
  describe("migration 0186 is safe to apply twice", () => {
    it("applies nothing the second time and leaves the schema alone", async () => {
      ...
      expect(again.skipped).toContain("0186_a_village_says_what_it_is_for.sql");
 ...
 * insert instead of losing the row to strict MySQL, and that 0187 applied
...
describe.skipIf(!configured)("the member's own needs card", () => {
  ...
  describe("migration 0187 is safe to apply twice", () => {
    it("applies nothing the second time and leaves the schema alone", async () => {
      const schema = new URL(db.url).pathname.replace("/", "");
      const shapeOf = async () => {
        const [cols] = await db.conn.query<any[]>(
      ...
      expect(again.failed).toBeNull();
      expect(again.applied, "a second run applies nothing").toEqual([]);
      expect(again.skipped).toContain("0187_a_member_says_how_they_are.sql");
      expect(await shapeOf()).toEqual(before);
    }, 120_000);
