 ...
 * the password itself. Sign-in stopped accepting those, so a member still
 * holding one cannot satisfy a password check at all. Sessions minted before
 * that change keep working, so such a member can be signed in, reach this gate
 * and be told their password is wrong when it is right.
 *
 * Sign-in deliberately does NOT say this. It answers every failure with one
 * sentence, because a different answer for a known address tells a stranger
 * which addresses exist. This gate only ever answers somebody who already
 * holds a session, so it can afford to name the way out.
 */
export const PASSWORD_NEEDS_RESET =
  "Your password needs setting again before you can confirm this. " +
  "Use Forgot password on the sign-in page, then come back.";

export function storedHashNeedsReset(storedHash: string | null | undefined): boolean {
