...
describe.skipIf(!DB_CONFIGURED)("a signed-in account names its own type", () => {
  ...
  it("is admitted when the village says yes, and only then", async () => {
    ...
    // What the route told the desk it did. The panel reads this to say which
    // of the two accepts just happened, and the stranger's case below is the
    // other half: the same field reading false is what makes true mean
    // something.
    expect(moved.json?.admitted, "a signing that names its signer admits them").toBe(true);
