...
export function registerMemberCard(app: Express, deps: MemberDeps): void {
   ...
   * exists, and the column in 0205 admits one value anyway, so a silent
   ...
   * Below the floor a need the village took on keeps its row with both
   * numbers null and `suppressed` true, which a screen prints as a sentence
   * about why it has nothing to show. A need the village never took on has no
   * row until its answers reach the floor, because its label can be one
   * member's own words.
   */
