...
by `drizzle/0203_voice_that_waned.sql` that is not a faucet and only ever
...
Since **`0202`** (2026-09-04) four of the seven tokens carry `decimals = 2` and
three carry `0`.
...
corrected in the same change that landed `0202`, along with six more the count
had missed.
...
`0202` selects BY KIND and never by a list of slugs, and `ensureStayToken` and
...
1. **Assert the token's stored amounts are all gone before changing its
   decimals.** `0202` does this and REFUSES, naming the token in the error text
...
**"ISSUED SUPPLY IS ZERO" IS NOT A WIDE ENOUGH QUESTION, AND `0202` ASKS A WIDER
...
an open redemption. `sys:voice-decay` had been missing since `0203` landed it,
and `sys:redemption-hold` and `sys:redeemed` arrived in `0201` today and did not
...
1. **Decimals. SETTLED 2026-09-04, AND SHIPPED. This entry stated the ruling
   ...
   `0202` carries it. Four of the seven tokens hold `decimals = 2` and three hold
...
|---|---|---|---|---|
| R1 needs | 5 keys `needs.*` in `shared/gameVariables.ts` | `village_needs`, `need_links` from `drizzle/0204_a_village_says_what_it_is_for.sql`; `shared/needs.ts` names 10 needs | `server/routes/needs.ts`, `client/src/components/admin/NeedsPanel.tsx` (which re-exports `NeedsSetupStep`) | built |
| R3 voice decay | `economy.voice_decay_pct` default 1, `economy.voice_decay_basis` | `sys:voice-decay` seeded by `drizzle/0203_voice_that_waned.sql` with `faucet` 0; source `voice_decay`; `decayVoice` called from `runSettlement` | The Mint dials, and `publicSupply`'s `waned` beside `issued` | built |
...
links live in `village_needs` and `need_links` from migration 0204, and
...
seeded by migration 0203 with the faucet flag at 0, because a faucet flag there
