#!/usr/bin/env node
/**
 * docs/CAPABILITIES.md still says what the permission model does.
 *
 * `scripts/generate-capabilities-doc.mjs` writes that document out of
 * `shared/capabilities.ts`, `shared/modules.ts` and `shared/gameConfig.ts`.
 * This runs the same generator and compares the result with the committed
 * file. They differ when somebody changed the model without regenerating, or
 * edited the document by hand, and both of those are the same failure: a
 * founder reading a permission document that is quietly wrong about who can do
 ...
 * pass in CI. The comparison strips carriage returns, because the same
 * carriage-return class has produced a per-machine answer in this repository's
 * guards more than once (see scripts/check-brand-refs.test.mjs).
 *
 * Usage:
 *   node scripts/check-capabilities-doc.mjs
 *   node scripts/check-capabilities-doc.mjs --list   print what the generator reads
 */
import fs from "node:fs";
import path from "node:path";
import { DOC_PATH, ROOT, SOURCES, generateDetailed } from "./generate-capabilities-doc.mjs";

const REL = path.relative(ROOT, DOC_PATH).replace(/\\/g, "/");
const REGENERATE = "node scripts/generate-capabilities-doc.mjs";
...
async function main() {
  ...
  if (!diff) {
    report([
      `Capability doc guard passed. ${REL} matches the code: ` +
        `${facts.counts.keys} capability keys, ${facts.counts.steps} steps in the gate ` +
        `(${facts.steps.map((s) => s.source).join(" then ")}), ` +
        `${facts.counts.voices} of them a warning badge may never deny.`,
    ]);
    return;
  }
