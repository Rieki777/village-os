...
- **canvas-baseline lane (L4, every canvas block on record before a Birthing), 2026-09-24: holds
  0222** for `drizzle/0222_a_village_reads_its_canvas.sql` on `wt/canvas-baseline`: one new
  table, `canvas_readings`, `CREATE TABLE IF NOT EXISTS`, append-only, one non-unique index on
  `(block_id, created_at)`, no `ALTER`, no foreign key. Expand-only and replay-safe by
  construction; the previous release neither reads nor writes it. **The brief that opened this
  lane said the next free number was 0221, and it is taken:** `origin/wt/saberra-boundary-fix`
  (commit `d6906e9`) holds `drizzle/0221_a_seat_carries_what_a_module_knows.sql`, and the same
  file sits on disk in `wt-holders`. Measured four ways on 2026-09-24 after
  `git fetch origin --prune`, immediately before the file was created: the `--diff-filter=AR`
  scan over every ref reaches **0221**; the `drizzle/` directories on disk across every sibling
...
| A CI step for `scripts/check-test-pool-timezone.mjs` | safety (owns `ci.yml`) | 2026-09-23, govbuild lane (`wt/testpool-timezone`) | New ratchet, same refuse-to-raise discipline as `check-theme-literals.mjs` and `check-file-lines.mjs`, gating test pools that are missing the MySQL session pin. Exact step to add, beside the other ratchets before the Build step: `- name: Test pool timezone` then `run: node scripts/check-test-pool-timezone.mjs`. No env, no DB, no new secret, sub-second. Committed baseline (`scripts/test-pool-baseline.json`) is **187**, measured across 318 test files, 177 of which hold a pool; 11 are pinned and 6 carry an on-the-line `test-pool-ok:` waiver. **The gate is ALREADY ENFORCED without any ci.yml change**, by `server/testPoolRatchet.test.ts` under the existing `pnpm test` step, which drives the REAL shipped script at throwaway fixture trees through its own `--dir` and `--baseline` flags: 9 cases covering the refusal, the refusal to raise its own baseline (with the file left byte-identical), the permitted downward write, subdirectory walking, the on-the-line waiver rule and that a waiver on the line ABOVE does nothing. Filing the ci.yml step anyway so the guard fails branch protection rather than only the suite. **Why a ratchet and not a wall:** 187 files would have to change to reach zero, which would conflict with every lane in flight for a fidelity win in files that mostly never compare a clock, so new pools are born pinned and the rest come across as their files are touched. |
...
| 2026-09-15 | google-member-exit lane (for `amora-b0`) | `server/index.ts`, **net +1 counted line** (27169 to 27170 against a baseline of 27507; routes unchanged at 392, baseline NOT moved): the password check in `POST /api/profile/request-exit` and `POST /api/profile/delete-account` becomes one `confirmIdentity` call each, the logic being `server/lib/identityConfirm.ts`; one gate constant and its comment sit beside `googleSignInAvailability`; one import; `authedUser` handed to the Google route registration. Also `server/routes/authGoogle.ts` (a confirm branch in the callback, plus `GET /api/auth/confirm-methods`) and `server/lib/oauthGoogle.ts` (the signed state carries the action). `docs/GOVERNANCE.md` and its lineage regenerated only. | `wt/google-member-exit` | HELD, and see the follow-up row below for the state after #270 landed. **NO MIGRATION TAKEN**: the single-use record for a Google confirmation lives in `users.prefs` beside `googleLink`, so nothing needs a column. **`server/lib/erasure.ts` and the erasure routes are NOT touched**, which is deliberate: #238 (`wt/erasure-files`) and #247 (`wt/failed-actions`) both own that file in the landing batch, so this lane gates at the route and keeps its logic in a new lib. New e2e windows 1200-1599 and 1600-1999 (`server/googleMemberExit.routes.e2e.test.ts`), taken from the only two gaps `check-e2e-ports` leaves; 3000-4001 was passed over because local MariaDB holds 3307. |
| 2026-09-15 | google-member-exit lane (for `amora-b0`) | follow-up to the row above, after the landing batch | `wt/google-member-exit` | **MERGED MAIN at `ee38104` (#270).** Worth knowing how this presented: the pull request had gone CONFLICTING, and GitHub then creates NO pull_request run at all, so the branch showed a passing push `verify` and nothing else. The absence looked like quiet, not like a red. Only the two generated governance documents conflicted, and both were resolved the one way they can be, by regenerating from the merged code. The index ratchet now reads **27171 lines / 392 routes / 48 route modules** against baselines 27507 / 399, still unmoved. `server/lib/erasure.ts` arrived from #238 and #247 and this lane still does not touch it; the delete route it gates now calls that newer code, and the Google exit e2e, `authGoogle`, `needsTombstone`, `founderEnds` and main's own `erasure` unit tests were re-run green on the merged tree. |
...
| 2026-09-23 | merge-conflict session (dependency round) | **The test runner every lane is judged by: `vitest` 3.2.7 -> 4.1.11**, plus the development and production dependency groups and seven workflow files (`actions/checkout` v4->v7, `setup-node` v4->v7, `pnpm/action-setup` v4->v6, `codeql-action` v3->v4). No `.ts`/`.tsx` file changes | `wt/deps-round-land` | RECORDED (a measurement, not a claim). **Landed as its own round rather than inside a feature batch, because vitest is the instrument that judges every other claim in a batch**: a red would be unattributable between a dependency and a feature, and a green would be worse, since a runner that quietly collects fewer files passes over a smaller denominator and waves through everything riding with it. Measured two independent ways, both over identical test sources, which is the only condition that makes either meaningful. (1) File-set difference by the quests lane, naming every differing path rather than reconciling counts: vitest 4 collected **every file** vitest 3 did, the only two differences being that lane's own new files, present on one tree by construction. (2) Same-tree totals from CI under one reporter and command: batch 5 ran **529 test files and 8064 tests** under 3.2.7, and `git diff --name-only f94f897..HEAD` filtered to `.test.ts`/`.test.tsx` returns **zero**, so this round had to report the same two numbers. **A lane whose tests now behave differently should suspect the runner, not its own change.** One risk checked and cleared: the `checkout` bump could have hollowed out every gate that diffs against a base ref by shallowing the clone, and those gates would have gone GREEN, not red — `fetch-depth: 0` is set explicitly wherever it matters, this round touches none of those lines, and the count is 2 before and 2 after. |
...
| 2026-09-24 | hand-answers lane (merged batches 12 AND 13) | `server/routes/powerHands.ts` owns the seat vote OUTRIGHT; `server/index.ts` MEASURED with `check-server-index-size.mjs` after both merges: **27077 of a 27089 baseline (12 lines of headroom HANDED BACK) and 390 of 390 route registrations (0 left)**. `powerHands.ts` is 748 of its 2000-line route-module cap. Baseline NOT moved. New: `server/lib/holdersNow.ts` (70 lines) and `rolesCarryingCapability` in `server/lib/roleGrants.ts` | `wt/hand-answers`, PR #336 | RECORDED (a measurement, not a claim), and it CORRECTS my own row above, which is the only row here I may rewrite. #371 moved `POST /api/governance/role-seats` out of `server/index.ts` into `registerSeatVote` with the body inline while this branch had lifted the same body into a local `openSeatVote` there. **Both wanted one spelling, so the 195-line block named in my earlier row is GONE from `server/index.ts` and `openSeatVote` lives inside `registerSeatVote`, which RETURNS it.** Batch 13 then lowered the index baseline to zero slack, and this branch was net +38 there, so it paid by moving the badge-plane read and the roles-carrying test OUT of the monolith rather than by trimming reasoning out of it. **NEITHER REGISTRATION MOVED and no gate can see why**: the hand door registers ~5,300 lines above the `requireModule("governance")` mount and the seat vote below it, Express matches in registration order, and putting them side by side would take the governance module's lifecycle gate off a governance route with every gate still green. `deferredSeatVote()` in `server/routes/powerHands.ts` carries the opener between them, and its header is where a lane editing either registration should read first. **The control:** `server/routes/powerHands.test.ts` wires the real `registerSeatVote` to the real `register` and asserts both doors answer one refusal with the SAME sentence; breaking the shared function turns 3 named tests there and 8 in `steward.routes.e2e.test.ts` / `stewardSeat.routes.e2e.test.ts` red, while the 17 older tests in that file, which stub `openSeatVote`, stay GREEN, so they were never able to see a twin. |
...
| 2026-09-24 | brief-audience lane (L3, defect 12) | `server/index.ts` LINE-NEUTRAL: the `PUT /api/admin/brain/:section` audience line and its audit text, the `GET /api/village/brain` rows line (an account without `membershipGranted` reads only the strangers' allowlist) and three comment lines change in place, and two names join the existing `./lib/villageBrain` import line. Measured before at **27070 of 27089 lines, 390 of 390 routes**; no route added, no baseline moved. Logic lives in `server/lib/villageBrain.ts` (sticky audience in `briefWrite`, the strangers' allowlist in `briefForPublicPrompt`) | `wt/brief-audience`, off `6f64972` | HELD. No migration, no capability key, no game variable, no baseline. A lane opening canvas sections to members (or adding a brief section) should read the allowlist header in `villageBrain.ts` first: a new section never reaches the public guide until somebody adds it there by name. |
| 2026-09-24 | intake-private lane (L1) | `server/index.ts`, NET DOWN: `POST /api/exit/restorative-intake` leaves the monolith for `server/routes/restorativeIntake.ts` (same path, registered at the same spot), with its logic in `server/lib/restorativeIntake.ts`. So one route registration and about 25 counted lines are HANDED BACK; the baseline is NOT lowered, so the headroom goes to whoever lands next. `GET /api/exit-policy` gains one field in place (`restorative.intakeRole`). Also `server/lib/notify.ts` (one per-kind rule: a `restorative_intake` email carries the title alone), `client/src/pages/ExitPolicy.tsx` (copy), `server/loop.e2e.test.ts` (the intake case, plus a test-only fetch preload that captures Resend mail), and `docs/GOVERNANCE.md` plus its lineage regenerated only | `wt/intake-private` | HELD. No migration, no baseline moved, no capability key, no game variable, no Admin.tsx. Rye authorised deploying this to live Amora once green. |
| 2026-09-24 | honest-copy lane (L2) | `server/index.ts` LINE-NEUTRAL, three lines edited in place and none added: the `./lib/exitPolicy` import list widens by one name on an existing line, `GET /api/exit-policy` gains one field (`platformWording`, the keys of terms still in the platform's words) on its existing `configured` line, and the `POST /api/admin/launch/propose` governance-off refusal stops telling every fork it decides on Hypha. `client/src/pages/Admin.tsx` LINE-NEUTRAL: the go-live wizard's retired `JOURNEY_PASSWORD` line is rewritten in place. Also `server/lib/exitPolicy.ts` (`platformDefaultTermKeys`, which `platformDefaultTerms` now maps to labels) | `wt/honest-copy`, off `origin/main` `6f64972` | HELD. No migration, no route registration, no baseline moved. A lane editing `GET /api/exit-policy` or `platformDefaultTerms` rebases over this. |
...
| 2026-09-26 | canvas-text lane (Wave 1: the canvas's own wording, and the printable workbook) | **NO MIGRATION, no server file, no route in `server/index.ts`, no ratchet baseline.** Claimed: NEW `shared/governanceCanvasText.ts` (the canvas's own words keyed by `CanvasBlockId`, and `CANVAS_CREDIT`, which moves here from `shared/governanceCanvas.ts` and is RE-EXPORTED there under the same name, so no importer changes); `shared/governanceCanvas.ts` (the re-export and its header comment only: ids, levels, prompts and the validator are untouched); `client/src/components/canvas/CanvasBlockCard.tsx` and `CanvasBaseline.tsx` (the card shows the canvas's question and description beside our prompts; the radar legend reads the canvas's scale words; one link to the workbook); NEW `client/src/components/canvas/CanvasWorkbookSheet.tsx`, `client/src/pages/CanvasWorkbook.tsx`, `client/src/lib/canvasWorkbook.ts`; `client/src/lib/canvasCopy.test.ts` (its pinned file list grows by the workbook, and the workbook joins its sweep); `client/src/App.tsx` (one lazy import, one route `/canvas/workbook`, one `PAGE_TITLES` line); `client/src/pages/Governance.tsx` (one short section linking the workbook, its second door for `check-route-reachability`); `docs/prototypes/grounds-v0.html` `SITE_PAGES` RE-DERIVED by its owning script for the one new route | `wt/canvas-text`, off `origin/wt/canvas-build` `5eeacae` | HELD. Merges into `wt/canvas-build`, never main. A Wave 1 lane editing `CanvasBaseline.tsx`, `CanvasBlockCard.tsx` or `App.tsx` rebases over this; every change there is additive except the card's question line, which now leads with the canvas's question and moves ours into "Our questions to talk through". |
