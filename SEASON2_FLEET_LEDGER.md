...
| 2026-09-26 | fresh-seed-audit lane (Wave 1) | `server/seeds/site-content-seed.json` (milestones and FAQs emptied, visit and investor terms "To be confirmed"), `circles-seed.json` (emptied), `content-seed.json` (emptied), `roles-seed.json` (Treasury loses its Hypha clause), `quests-seed.json` (the retreat-centre quest removed, its `practitioners` gate moved to the healing-arts quest, whose description loses "plant medicine ceremonies"); `server/lib/trainingStarter.ts` (descriptions stop claiming the practice for the village); TWO sections of `server/loop.e2e.test.ts` (S19-S23 forms its own three circles before the map turns on; the command-centre stale-milestone check writes its own two milestones); `server/forkPublish.e2e.test.ts` (two new describe blocks, same boot, same port window); `docs/FORK_RUNBOOK.md` (the seeds lines) | `wt/fresh-seed-audit`, off `origin/wt/canvas-build` `5eeacae` | HELD. **No migration, no `server/index.ts`, no `Admin.tsx`, no baseline, no capability key, no game variable.** Every seed touched is applied only to an empty table or document, or is a read-time default that live Amora has already overridden with a stored document (measured by reading its public routes on 2026-09-26), so no running village changes. A lane editing `loop.e2e.test.ts` S19-S23 or the command-centre section rebases over this: neither may assume a seeded circle or milestone any more. |
