...
| 2026-09-09 | correspondence (`11eceb`) | `scripts/check-identity-keys.mjs` (+ its test) and `server/lib/capabilityRegistry.test.ts` | `wt/correspondence` | CLAIMED. Adding the REVERSE direction to two guards that are handed their subjects instead of discovering them. `check-identity-keys` iterates `IDENTITY_KEYS` and nothing iterates `GAME_CONFIG`, so it is a BLOCKING CI step (`ci.yml:510`) printing `every key outside the pending list is empty or platform-neutral` while checking 29 of the config's 33 string keys. One of the four it cannot see is `season.timezone = "America/Costa_Rica"`, which is the survivor of the leak in section 17. `capabilityRegistry.test.ts` gets the matching assertion: a key in `NOT_YET_WIRED` claims it gates nothing and nothing checks that. Touching NO other lane's code; the vouch and archetype findings are routed to their owners, not fixed here. |
| 2026-09-09 | correspondence (`11eceb`) | `scripts/check-identity-keys.mjs`, `capabilityRegistry.test.ts`, nine `docs/modules/*.md` | `wt/correspondence` | **RELEASED, and one live defect closed.** The identity guard is a required CI step that iterated IDENTITY_KEYS while nothing iterated GAME_CONFIG, so it watched 29 of 33 string keys and printed "every key outside the pending list is empty or platform-neutral" either way. The four it could not see included `season.timezone = "America/Costa_Rica"`, the survivor of section 17's leak. Held as known-pending with its reason; blanking it would repeat section 17's actual mistake on a live village. **PENDING_CEILING rose 1 to 2, the first rise ever**, on Rye's ruling that the zone comes from founder setup. Also: `capabilityRegistry.test.ts` now asserts NOT_YET_WIRED keys really gate nothing, matching `hasCapability("<key>"` and not the bare key, because `member.vouch` appears in server/index.ts inside lists two proposal types REFUSE to move. |
| 2026-09-09 | correspondence (`11eceb`) | **FOR THE FIRST-HOUR LANE**: `server/lib/vouches.ts`, `membership.vouches_required` | `wt/vouch-derivation` (on `wt/first-hour` @ `5602e7f`) | **READY TO TAKE, and it does not touch your worktree.** The number 3 had THREE homes on your branch: `DEFAULT_VOUCHES_FOR_MEMBERSHIP`, the dial's `default`, and `minElectorate` on VILLAGE_LAUNCH, all three carrying the same sentence about a village launching with three so a fourth can be admitted, with nothing comparing them. Both now read `SUBJECT_THRESHOLDS[VILLAGE_LAUNCH].minElectorate`, the idiom gameVariables.ts already uses for the two mint-rule dials; proved by moving the bar to 5 and watching both follow. **`min` is 0 rather than 1, on Rye's ruling of 2026-09-09**: a floor of 1 is a mandate, not a default, and thirteen forks inherit it. Default stays 3. `membership.vouch_threshold` DELETED (three declarations, no readers, verified against a positive control) so a founder does not meet two dials with nearly the same label, one dead. Five docs regenerated LAST. All gates and 129 tests green. |
| 2026-09-09 | correspondence (`11eceb`) | **FINDING FOR THE ADMIN LANE**, not claimed: a timezone field | (unclaimed) | `season.timezone` is settable by `PUT /api/admin/seasons` (`server/index.ts:3596`, `raw.timezone ?? def.timezone`) and NO client calls it, so the only value any village has is the platform default. **Rye ruled 2026-09-09: the zone comes from the founder at setup.** Suggest `Intl.DateTimeFormat().resolvedOptions().timeZone` for now, which the browser already knows at zero bytes and no dependency, and derive from `village_land.centre_lat/centre_lon` once the land screen exists (the columns and route are live, the admin surface is not). Until the field ships, `season.timezone` sits on KNOWN_PENDING holding the ceiling at 2. It graduates and the ceiling returns to 1 the day a founder can type it, or the day somebody confirms the live village's season document already carries its own. `server/index.ts:11288` calls it "the zone every viewer reads in", so this is every timestamp, not only seasons. |
| 2026-09-09 | correspondence (`11eceb`) | **FINDING FOR THE ARCHETYPES LANE**, not claimed: `server/lib/economySeed.ts` | (unclaimed) | The file contradicts itself 165 lines apart. L25 says the five class names are "the village's vocabulary and renameable"; L192 says "Until that editor ships, a rename is an admin act on the row". Verified at `c04307e`: no `renamed` column in any migration, no admin write route for archetypes, no client surface. The L192 mechanism ("restored from the row once `renamed` is set") describes a column that does not exist, while the OUTCOME is accidentally right, because the ON DUPLICATE KEY UPDATE omits `name` and so preserves a hand-edited one. Either ship the editor or make L25 say what is true today. |
| 2026-09-09 | correspondence (`11eceb`) | **FINDING, six module contracts**: `crowdpool.md`, `crowdpool-dashboard.md`, `gratitude-feed.md`, `internal-exchange.md`, `stays.md`, `token-registry-ledger.md` | (unclaimed) | Nine module contracts were given `describes:` markers derived from the code they already cite. These six could not be: they name only shared furniture (`shared/capabilities.ts`, `shared/gameVariables.ts`, `server/lib/ledger.ts`) and never their own implementation. CLAUDE.md's reading order sends every lane to these before changing a module, and a contract that never names its own code cannot be checked against it by anybody, tool or person. Naming two or three real files in each is a five-minute job for whoever owns that module and it is the difference between a watched contract and an unwatched one. |
| 2026-09-09 | correspondence (`11eceb`) | **CORRECTION to my own row above**: the capability gate scan | `wt/correspondence` @ `442b057` | **THE ROW ABOVE SAID THE ASSERTION MATCHES `hasCapability("<key>"`. THAT VERSION WAS WRONG AND IT FAILED GREEN.** The governance lane (`b7f9ef`) caught it within the hour and I verified before believing it: there are THREE spellings, 59 `hasCapability("`, 7 `mayAct(req, "`, and 2 `mayAct(req, STEWARD_VETO)` where the argument is a CONSTANT (`mayAct` is a real gate at `server/index.ts:3224`; `STEWARD_VETO` is `"steward.veto"` at `server/lib/stewardship.ts:123`). Matching one spelling missed nine gated routes, so a WIRED key read as unwired and the assertion would have passed while a live gate sat in a list promising it gates nothing. **My enumeration of spellings was itself a hand-kept list, written in the commit that fixed a hand-kept list of keys.** The fix is a discovered SET: both helpers scanned, constants resolved by finding `export const NAME: Capability = "key"`, and the count held to a floor of 19 so a fourth helper shrinks coverage LOUDLY. `generate-governance-doc.mjs`'s route classifier learned this first and the comment now points at it. **The general lesson, and it is the one worth carrying: whoever is fixing this class is standing inside it, and the direction they cannot see is the one their own tool defines.** |
