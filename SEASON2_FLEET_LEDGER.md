...
| 2026-09-24 | brief-audience lane (L3, defect 12) | `server/index.ts` LINE-NEUTRAL: the `PUT /api/admin/brain/:section` audience line changes in place and one name joins the existing `./lib/villageBrain` import line. Measured before at **27070 of 27089 lines, 390 of 390 routes**; no route added, no baseline moved. Logic lives in `server/lib/villageBrain.ts` (sticky audience in `briefWrite`, the strangers' allowlist in `briefForPublicPrompt`) | `wt/brief-audience`, off `6f64972` | HELD. No migration, no capability key, no game variable, no baseline. A lane opening canvas sections to members (or adding a brief section) should read the allowlist header in `villageBrain.ts` first: a new section never reaches the public guide until somebody adds it there by name. |
