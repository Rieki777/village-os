...
| 2026-09-26 | canvas-season lane (Wave 1: the season file, and room in Journey) | `client/src/pages/JourneyToLaunch.tsx` (`TestRun` and `LaunchGuide` move to `client/src/components/journey/`, no behaviour change, and the Canvas view renders `CanvasView`); `client/src/pages/ProjectHistory.tsx` (`EconomicsView` moves to `client/src/components/journey/EconomicsView.tsx` and is re-exported, so the page still renders it); `server/index.ts` (ONE import of `./routes/canvasSeason` and ONE register call, both exempt from the ratchet, plus ONE counted line in the `GET /api/admin/command-centre` payload, `hyphaSpace`, so "Copy for Hypha" shows only when a Hypha space is set); `server/repos/appConfigDocs.ts` (a writer and a remover beside `readConfigDocument`); the `canvas-season` key in `app_config`; `client/src/components/canvas/` (`CanvasSeason.tsx`, `CanvasView.tsx`; `CanvasBaseline` takes the week's focus) and the pinned file list in `client/src/lib/canvasCopy.test.ts`; `docs/seasons/`; `docs/FORK_RUNBOOK.md` (appended); e2e port window **4050-4149** for `server/canvasSeason.routes.e2e.test.ts` (clear by `check-e2e-ports.mjs` on this branch; 7400, 9500 and 2000 were each taken or held a port `fetch()` refuses) | `wt/canvas-season`, off `origin/wt/canvas-build` `5eeacae` | HELD. No migration (the season is an `app_config` document), no capability key (the pen is `story.tell` through the one gate, as `server/routes/canvas.ts` asks it), no game variable, no baseline moved: `ProjectHistory.tsx` falls under its file-lines baseline and the baseline is left where it is, so the headroom goes to whoever lands next. |
