...
| 2026-09-21 | token-hash lane (for `amora-b0`) | `server/index.ts`, **net 1 counted line DOWN** (27189 to 27188; baseline 27507 not moved): `POST /api/auth/set-password` asks `setPasswordLinkRefusal` (server/lib/memberTokens.ts) on a plain read and again inside the locked row update, and the two bootstrap mints plus the admin send-password-link mint pass `tokenVersion` where they passed the stored hash; one import swapped. Also `server/routes/authRecovery.ts` (the forgot-password mint), `server/lib/memberTokens.ts`, and `docs/GOVERNANCE.md` with its lineage regenerated only | `wt/token-hash-alert` | HELD. **NO MIGRATION TAKEN**: the link is bound to the existing `users.token_version`, which every password-hash write already bumps. CodeQL alert 36's substance: no set-password link carries anything derived from a password hash any more. A lane minting or reading set-password links rebases over this: `makeSetPasswordToken`'s third argument is now the tokenVersion and throws on a string, and the claim's purpose is now `set-password-2`, so the previous release refuses these links outright and a rollback fails closed. |
