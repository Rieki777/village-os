...
| 2026-09-26 | season-village lane (Wave 1, the Season Two test village) | the local MariaDB schema **`village_season_qa`** (a scratch village dropped and rebuilt by `scripts/qa/season-village/boot.mjs --fresh`, which refuses to drop a schema of that name it has no record of creating), default port **`38471`** (outside every e2e window, below the Windows dynamic range and off the fetch blocked-port list; `QA_PORT` overrides it), and the new directory `scripts/qa/season-village/` | `wt/season-village`, off `origin/wt/canvas-build` `5eeacae` | HELD. No migration, no route, no `server/index.ts` line, no baseline, no dependency. The harness writes its state, tokens and screenshots under `QA_OUT_DIR` (default the OS temp dir) and refuses a directory inside the repository. A later wave extends `surfaces.json` rather than the scripts. |
